...
export async function recordPending({ name, amount, anon, deviceId, orderId, ts }) {
  // Push to pending list with timestamp
  await redis.lpush('donation:pending', JSON.stringify({ name, amount, anon, deviceId, orderId, ts }));
}

// Record successful donation: update total, leaderboard, recent list
export async function recordSuccess({ name, amount, anon, deviceId, orderId }) {
  ...
  // maintain only latest 5 entries
